    ...
    <div className='grid min-h-svh place-items-center p-4'>
      <div className='w-full max-w-sm space-y-6'>
        <div className='flex flex-col items-center gap-2 text-center'>
          ...
          <h1 className='font-heading text-xl font-bold tracking-tight text-heading'>{t('auth.signUpTitle')}</h1>
        </div>
        ...
        <Card className='space-y-4 rounded-2xl p-6 shadow-md'>
          <form className='space-y-3' onSubmit={handleSubmit(onSubmit)} noValidate>
            <div className='space-y-1'>
              ...
              {errors.displayName ? <p className='text-xs text-danger'>{t('form.required')}</p> : null}
            </div>
            <div className='space-y-1'>
            ...
            <Button type='submit' className='w-full' disabled={isSubmitting}>
              {isSubmitting ? t('states.loading') : t('auth.signUpAction')}
            </Button>
          </form>
